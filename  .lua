...
espAttach = function(obj, tag, color, isChar)
    ...
    local root = obj:FindFirstChild("HumanoidRootPart") or (obj:IsA("Model") and obj.PrimaryPart) or obj:FindFirstChildWhichIsA("BasePart") or obj:FindFirstChild("Base") or obj:FindFirstChild("Main")
...
-- TP to Items
------------------------------------------------------------------------
local secTPItems = tabVisual:Section({ Title = "TP to Items", Opened = true })

local _tpOrigCFrame = nil

local function tpItemsGetList(filter)
    local items = {}
    for _, obj in ipairs(svc.WS:GetDescendants()) do
        if (filter == "All" and (obj.Name == "BloxyCola" or obj.Name == "Medkit"))
        or (filter == "BloxyCola" and obj.Name == "BloxyCola")
        or (filter == "Medkit" and obj.Name == "Medkit") then
            local part = obj:IsA("BasePart") and obj
                or obj:FindFirstChildWhichIsA("BasePart")
            if part and not espItemHeld(obj) then
...
local _tpItemFilter = "All"
secTPItems:Dropdown({ Title = "Item Filter", Flag = "tpItemFilter", Values = {"All","BloxyCola","Medkit"}, Default = "All", Callback = function(v) _tpItemFilter = v end })

secTPItems:Button({ Title = "TP to Items", Callback = function()
    local char = lp.Character
    local hrp  = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
    local items = tpItemsGetList(_tpItemFilter)
    if #items == 0 then return end
    -- save original position
    _tpOrigCFrame = hrp.CFrame
    local idx = 1
    local function tpNext()
        if idx > #items then return end
        local part = items[idx]
        if part and part.Parent then
            hrp.CFrame = CFrame.new(part.Position + Vector3.new(0, 3, 0))
        end
        idx = idx + 1
        if idx <= #items then
            task.delay(0.5, tpNext)
        end
    end
    tpNext()
end })

secTPItems:Button({ Title = "Return to Original Position", Callback = function()
    local char = lp.Character
    local hrp  = char and char:FindFirstChild("HumanoidRootPart")
    if hrp and _tpOrigCFrame then
        hrp.CFrame = _tpOrigCFrame
        _tpOrigCFrame = nil
    end
end })
...
do
    ...
    local elliotRequireAnim = true  -- renamed to "Require Remote" in UI
    local elliotUseAnimTrigger = false
    local elliotTrackedAnimIds = { ["114155003741146"] = true }
    local elliotAnimConn = nil
    ...
    local function elliotSetupChar(char)
        ...
        if elliotAnimConn then elliotAnimConn:Disconnect(); elliotAnimConn = nil end
        local animator = elliotHum:WaitForChild("Animator")
        elliotAnimConn = animator.AnimationPlayed:Connect(function(track)
            if not elliotUseAnimTrigger or not elliotEnabled then return end
            local id = tostring(track.Animation.AnimationId):match("%d+")
            if id and elliotTrackedAnimIds[id] then
                elliotIsThrowing = true
                elliotThrowTS = tick()
            end
        end)
    end
    if lp.Character then task.spawn(function() elliotSetupChar(lp.Character) end) end
    lp.CharacterAdded:Connect(function(c) task.spawn(function() elliotSetupChar(c) end) end)
    ...
    sec_014:Toggle({ Title = "Require Remote", Flag = "elliotReqAnim", Default = true, Callback=function(v) elliotRequireAnim=v end, Type = "Checkbox"})
    sec_014:Toggle({ Title = "Use Throw Animation", Flag = "elliotUseAnim", Default = false, Callback=function(v) elliotUseAnimTrigger=v end, Type = "Checkbox"})
    sec_014:Toggle({ Title = "Enable Elliot Aimbot", Flag = "elliotEnabled", Default = false, Callback=function(v)
        ...
        if v then
            elliotConnection = svc.Run.RenderStepped:Connect(function()
                ...
                local shouldAim = (not elliotRequireAnim and not elliotUseAnimTrigger)
                    or ((elliotRequireAnim or elliotUseAnimTrigger) and elliotIsThrowing)
