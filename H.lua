...
local function makeDocESP(obj)
    ...
    local function pulse()
        while h and h.Parent do
            ts:Create(h, TweenInfo.new(0.7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {FillTransparency = 0.6}):Play()
            task.wait(0.7)
            ts:Create(h, TweenInfo.new(0.7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {FillTransparency = 0.15}):Play()
            task.wait(0.7)
        end
    end
    task.spawn(pulse)
...
local function docESPStop()
    foundDoc = false
    for _, v in ipairs(workspace:GetDescendants()) do
        local h = v:FindFirstChild("CuteDocESP")
        if h then h:Destroy() end
        local b = v:FindFirstChild("CuteBillboard")
