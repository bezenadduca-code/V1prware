...
secTT:Slider({ Title = "Side Offset (st)", Flag = "ttSideOffset", Step = 0.25,
    Value = { Min = 0.0, Max = 5.0, Default = 2.0 },
    Callback = function(v) ttS.sideOffset = v end })

secTT:Slider({ Title = "Lunge Hold (s)", Flag = "ttLungeHold", Step = 0.005,
    Value = { Min = 0.05, Max = 0.50, Default = 0.25 },
...
local function ttFireDagger()
    local re = svc.RS.Modules.Network.Network:FindFirstChild("RemoteEvent")
    if not re then return end
    local buf = buffer.fromstring("\x03\x06\x00\x00\x00Dagger")
    pcall(function() re:FireServer("UseActorAbility", {[1] = buf}) end)
    pcall(function() re:FireServer("Dagger") end)
end
...
local function ttFlipToKillerBack(killerModel)
    ...
    holdConn = svc.Run.Heartbeat:Connect(function()
        ...
        local targetPos = khrp.Position
            ...
            + khrp.CFrame.RightVector * (ttS.sideOffset * sideSign)
        targetPos = Vector3.new(targetPos.X, hrp.Position.Y, targetPos.Z)
        hrp.CFrame = CFrame.lookAt(startPos:Lerp(targetPos, progress), khrp.Position)
    end)
end
...
task.spawn(function()
    pcall(function()
        local mainUI    = lp.PlayerGui:WaitForChild("MainUI", 15)
        local container = mainUI:WaitForChild("AbilityContainer", 15)
        local crouchBtn = container:WaitForChild("Crouch", 15)
        crouchBtn.MouseButton1Click:Connect(function()
            print("[TT] Crouch clicked, enabled:", ttS.enabled)
            if not ttS.enabled then return end
            local killer, _, dist = ttGetNearestKiller()
            print("[TT] Nearest killer:", killer, "dist:", dist, "range:", ttS.range)
            if not killer or dist > ttS.range then print("[TT] No killer in range, aborting") return end
            task.spawn(function()
                if ttS.triggerDelay > 0 then task.wait(ttS.triggerDelay) end
                local freshKiller, _, freshDist = ttGetNearestKiller()
                print("[TT] Fresh killer:", freshKiller, "freshDist:", freshDist)
                if freshKiller == killer and freshDist <= ttS.range then
                    print("[TT] Firing dagger...")
                    ttFireDagger()
                    task.wait(ttS.flipDelay)
                    ttFlipToKillerBack(freshKiller)
                else
                    print("[TT] Killer moved out of range after delay")
                end
            end)
        end)
    end)
...
secAntiTaph:Toggle({ Title = "Remove Blindness / Effects [Locked OFF]", Type = "Checkbox", Flag = "antiTaphEnabled", Default = false, Locked = true,
    Callback = function(on)
        antiTaphEnabled = false
        AntiTaph.remove()
    end
})
