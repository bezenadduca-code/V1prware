...
-- Logic from FINAL_AUTO_BLOCK using LinearVelocity (no MoveTo / no BodyVelocity)
-- Keeps Hutao's BLOCK_ANIMS IDs + hdtMoveSpeed / hdtDuration / hdtMissChance settings
...
local function combatHDTBeginDrag(killerModel, blockTrack)
    pcall(function()
        ...
        local bv = nil  -- BodyVelocity, created after miss roll

        local function restoreMovement()
            -- Zero velocity immediately so momentum doesn't carry on for ~1s
            ...
            bv = nil
            -- Restore AutoRotate so the game controls facing again
        ...
        -- Miss roll — bail before any state changes
        if combatRollMiss(combatS.hdtMissChance) then
            combatHDTDragging = false
            return
        end

        if not char or not killerModel or not hrp or not hum then return end
        ...
        hum.JumpPower     = 0
        hum.PlatformStand = false
        hum.AutoRotate    = false  -- we'll rotate manually each frame

        -- BodyVelocity — horizontal only (MaxForce Y=0 keeps gravity)
        ...
        -- Drive BodyVelocity toward killer each frame (FINAL AUTO BLOCK style)
        task.spawn(function()
            ...
            while not stopped and tick() < deadline do
                if not killerRoot or not killerRoot.Parent or not hrp or not hrp.Parent then break end

                local toTarget = killerRoot.Position - hrp.Position
                local horiz    = Vector3.new(toTarget.X, 0, toTarget.Z)

                if horiz.Magnitude <= 2.0 then
                    break  -- close enough
                elseif horiz.Magnitude > 0.01 then
                    local dir = horiz.Unit
                    bv.Velocity = Vector3.new(dir.X * speed, bv.Velocity.Y, dir.Z * speed)
                    -- Rotate HRP to face the killer
                    pcall(function()
...
sec_017:Slider({ Title = "HDT speed", Flag = "combatHDTMoveSpeed", Value = {Min=1,Max=60,Default=combatS.hdtMoveSpeed}, Step = 0.1, Callback=function(v) combatS.hdtMoveSpeed=v end
})

sec_017:Slider({ Title = "HDT delay", Flag = "combatHDTDelay", Value = {Min=0,Max=2.0,Default=combatS.hdtDelay}, Step = 0.01, Callback=function(v) combatS.hdtDelay=v end
})

sec_017:Slider({ Title = "Max Duration (s)", Flag = "combatHDTDur", Value = {Min=0.1,Max=3.0,Default=combatS.hdtDuration}, Step = 0.1, Callback=function(v) combatS.hdtDuration=v end
})
