...
do
    ...
    tabJaneDoe:CreateSection("Axe Lock-On")
    tabJaneDoe:CreateParagraph({ Title = "How it works", Content = "Detects when you use the Axe ability and locks your character to face the nearest killer for 1.7 seconds." })
    tabJaneDoe:CreateToggle({ Name="Enable Axe Lock-On", CurrentValue=false,
        Callback=function(on) jd_axeEnabled=on end })
    tabJaneDoe:CreateSlider({ Name="Lock Duration (s)", Range={0.5,3.0}, Increment=0.1, CurrentValue=jd_AXE_LOCK_DURATION,
        Callback=function(v) jd_AXE_LOCK_DURATION=v end })
