...
local docESP = { on = false, foundDoc = false, descConn = nil }

local function docESPClear()
    for _, obj in ipairs(svc.WS:GetDescendants()) do
        pcall(function()
            local h = obj:FindFirstChild("CuteDocESP");    if h then h:Destroy() end
            local b = obj:FindFirstChild("CuteBillboard"); if b then b:Destroy() end
            local a = obj:FindFirstChild("_DocESPAtt");    if a then a:Destroy() end
        end)
    end
    docESP.foundDoc = false
end

local function docESPMake(obj)
    if not docESP.on then return end
    if docESP.foundDoc then return end
    if not obj or not obj.Parent then return end
    if not obj:IsA("MeshPart") then return end
    if obj:FindFirstChild("CuteDocESP") then return end  -- already tagged, prevent double
    ...
    docESP.foundDoc = true  -- lock before any yields so nothing else slips through
    ...
    -- ✨ BILLBOARD — label is just "Document"
    ...
    print("[SAKIWARE] 🌸 Document ESP locked onto:", obj:GetFullName())
end
...
local function docESPStart()
    ...
    if docESP.descConn then docESP.descConn:Disconnect() end
    docESP.descConn = svc.WS.DescendantAdded:Connect(function(v)
        if docESP.foundDoc then return end
        task.wait(0.1)
        docESPMake(v)
    end)
...
local function docESPStop()
    docESP.on = false
    if docESP.descConn then docESP.descConn:Disconnect(); docESP.descConn = nil end
    docESPClear()
end
...
secDocESP:Button({ Title = "🔄 Force Rescan", Callback = function()
    if not docESP.on then return end
    docESPClear()
    docESPStart()
end })
...
task.spawn(function()
    ...
    task.defer(function()
        ...
        if docESP.on   then task.spawn(docESPStart) end
