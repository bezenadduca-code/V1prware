...
local function aiKillerLoop()
    ...
    local lockedTarget = nil -- locked survivor HRP
    ...
    while ai_enabled do
        ...
        -- Check if locked target is still alive and valid
        if lockedTarget then
            local targetHum = lockedTarget.Parent and lockedTarget.Parent:FindFirstChildOfClass("Humanoid")
            if not targetHum or targetHum.Health <= 0 or not lockedTarget.Parent or not lockedTarget:IsDescendantOf(svc.WS) then
                lockedTarget = nil -- target died or left, pick a new one
            end
        end
