...
do
    ...
    local function fireAbility(abilityName)
        ...
        if not net or not net.RemoteEvent then return false end
        
        pcall(function()
            net.RemoteEvent:FireServer(
                "UseActorAbility",
                {
                    buffer.fromstring("\x03\x05\x00\x00\x00" .. abilityName)
                }
            )
        end)
        return true
    ...
    local function scanCharacterForAbilities()
        -- Scan character for all RemoteEvents that might be abilities
        local char = lp.Character; if not char then return end
        
        abilityCache = {}
        
        local function scanChildren(obj, depth)
            if depth > 5 then return end
            for _, child in ipairs(obj:GetChildren()) do
                if child:IsA("RemoteEvent") then
                    abilityCache[child.Name] = child
                    print("[AI] Found ability RemoteEvent: " .. child.Name .. " at " .. child:GetFullName())
        ...
        scanChildren(char, 0)
        print("[AI] Total abilities found: " .. #abilityCache)
    end
    
    local function fireAbilityByName(abilityName)
        -- Try to find and fire ability RemoteEvent
        local char = lp.Character; if not char then return false end
        
        -- First check cache
        if abilityCache[abilityName] then
            print("[AI] Firing " .. abilityName .. " from cache")
            pcall(function() abilityCache[abilityName]:FireServer() end)
            return true
        end
        
        -- Search for it
        local function findAbility(obj)
            if obj.Name == abilityName and obj:IsA("RemoteEvent") then
                return obj
            end
            for _, child in ipairs(obj:GetChildren()) do
                local found = findAbility(child)
                if found then return found end
            end
            return nil
        end
        
        local ability = findAbility(char)
        if ability then
            print("[AI] Firing " .. abilityName)
            pcall(function() ability:FireServer() end)
            abilityCache[abilityName] = ability
            return true
        end
    ...
    local function aiKillerLoop()
        ...
        local abilityFireCooldown = 4 -- Cooldown between abilities (4 seconds)
        ...
        while ai_enabled do
            ...
            if targetHRP then
                ...
                -- Fire ability if within 13 studs and cooldown is ready
                if distance <= 13 then
                    local currentTime = tick()
                    if currentTime - lastAbilityTime >= abilityFireCooldown then
                        -- Try Slash first, then Punch
                        if not fireAbilityByName("Slash") then
                            fireAbilityByName("Punch")
                        end
