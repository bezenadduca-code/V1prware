...
local function aiKillerLoop()
    ...
    local abilityFireCooldown = 2
    local lockedTarget = nil
    local lastPathTime = 0
    local pathWaypoints = {}
    local waypointIndex = 1

    -- Heartbeat keeps reissuing MoveTo every frame so the 8s timeout never fires
    local moveConn = svc.Run.Heartbeat:Connect(function()
        ...
        -- Continuously issue MoveTo so it never times out
        if waypointIndex <= #pathWaypoints then
            local wp = pathWaypoints[waypointIndex]
    ...
    while ai_enabled do
        ...
        -- Validate locked target using strict check (parent folder + health)
        ...
        local targetHRP = lockedTarget.root
        if not targetHRP or not targetHRP.Parent then lockedTarget = nil; continue end

        local distance = (targetHRP.Position - hrp.Position).Magnitude

        -- Fire ability if close enough and off cooldown
        ...
        -- Recompute path every 0.5s or when waypoints run out
        local now = tick()
